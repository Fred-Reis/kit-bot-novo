...
Verificação pós-CodeRabbit: bot `bun run check` limpo (250 pass, 0 fail, 0 erros de lint); web `bunx tsc --noEmit` + `bun run lint` (0 erros) + `bunx vitest run` (132 pass, 0 fail) + `bun run build` limpo.
- [ ] Merge (Fred)
