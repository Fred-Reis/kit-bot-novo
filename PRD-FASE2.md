...
- [x] 6. Review local (`agent-skills:code-review-and-quality`, 5 eixos) — sem achados Critical/Important. RLS confirmada inerte; side effects best-effort não bloqueiam o fluxo principal; endpoint admin sem scoping por `ownerId` (mesmo padrão de todos os outros endpoints hoje, multi-tenancy fora de escopo). PR aberta, aguardando CodeRabbit + merge do Fred
- [ ] Merge (Fred)
